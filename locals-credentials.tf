...
locals {
  ...
  private_endpoint_connection_strings = coalesce(try(mongodbatlas_advanced_cluster.this.connection_strings.private_endpoint, null), [])
  cluster_connection_strings = merge(mongodbatlas_advanced_cluster.this.connection_strings, {
    private_endpoint = local.private_endpoint_connection_strings
  })
  conn_str_arr     = split("//", try(local.cluster_connection_strings.standard, ""))
  conn_str_srv_arr = split("//", try(local.cluster_connection_strings.standard_srv, ""))
  ...
  pvt_conn_str_arr     = length(local.private_endpoint_connection_strings) > 0 ? split("//", try(local.private_endpoint_connection_strings[0].connection_string, "")) : []
  pvt_conn_str_srv_arr = length(local.private_endpoint_connection_strings) > 0 ? split("//", try(local.private_endpoint_connection_strings[0].srv_connection_string, "")) : []
  ...
  cluster_credentials = try(var.settings.admin_user.enabled, false) ? {
    ...
    engine                        = "mongodbatlas"
    url                           = try(local.cluster_connection_strings.standard, "")
    srv_url                       = try(local.cluster_connection_strings.standard_srv, "")
